...
function withTimeout<T>(promise: PromiseLike<T>, ms: number): Promise<T> {
  return Promise.race([
    promise,
    new Promise<T>((_, reject) => setTimeout(() => reject(new Error('Timeout: servidor nao respondeu')), ms)),
  ]) as Promise<T>;
}
...
export function usePosts(isPublicFeedOrOptions: boolean | UsePostsOptions = false) {
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      ...
      if (pinErr) {
        const { data: raw } = await withTimeout(supabase.from('posts').select('*').eq('is_pinned', true).order('created_at', { ascending: false }), QUERY_TIMEOUT);
      ...
      if (regErr) {
        let fq = supabase.from('posts').select('*').eq('is_pinned', false).order('created_at', { ascending: false }).limit(PAGE_SIZE + 1);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (err) {
        const o = optionsRef.current;
        let fq = supabase.from('posts').select('*').eq('is_pinned', false).lt('created_at', last.created_at).order('created_at', { ascending: false }).limit(PAGE_SIZE + 1);
        if (o.isPublicFeed) fq = fq.eq('is_public', true);
        if (o.communityId)  fq = fq.eq('community', o.communityId);
        if (o.authorId)     fq = fq.eq('author', o.authorId);
        const { data: raw } = await withTimeout(fq, QUERY_TIMEOUT);
  ...
  const deletePost = useCallback(async (postId: string) => {
    try {
      const { error } = await withTimeout(supabase.from('posts').delete().eq('id', postId), QUERY_TIMEOUT);
